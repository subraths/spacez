...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 bg-background text-primary-3 z-50 shadow">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="md:hidden text-xl hover:cursor-pointer"><IoMdMenu /></div>
          </div>
        </div>
      </nav>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-background z-50 border-t border-t-gray-200">
        <div className="flex justify-around items-center h-16">
          {navItems.map((item) => (
            ...
            >
              <item.icon className={`w-6 h-6 ${pathName === item.href ? "text-primary-3" : "text-secondary-1"}`} />
              <span className={`text-xs mt-1 ${pathName === item.href ? "font-bold text-primary-2" : "text-secondary-1"}`}>{item.name}</span>
            </Link>
          ))}
        </div>
      </nav>
      ...
      <div className="hidden md:block h-16" /> {/* Top spacer for desktop */}
      <div className="md:hidden h-16" /> {/* Bottom spacer for mobile - add at page bottom */}
    </>
  );
}
